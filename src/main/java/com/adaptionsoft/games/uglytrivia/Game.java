	...
	public boolean wasCorrectlyAnswered() {
		if (inPenaltyBox[currentPlayer]){
			if (isGettingOutOfPenaltyBox) {
				...
				if (winner) {
					WinnersList.getInstance().add((String) players.get(currentPlayer));
					if (WinnersList.getInstance().getWinnings((String) players.get(currentPlayer)) > 3) {
						System.out.println(players.get(currentPlayer) + " is a champ!!!!");
					}
				}
				return winner;
			} else {
